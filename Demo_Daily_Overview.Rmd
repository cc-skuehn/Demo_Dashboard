...
```{r}
data_sets <- c('data_hamburg','data_berlin','data_munich')
...
base_data <- reactive(get(input$dataset)) # Figure out how to get the data out of this reactive thing
reporting_date <- reactive(get(input$datum))
#output$reference_week <- calculate_reference_week(output$base_data)
```
...
```{r}
#base_data <- reactive(get(input$dataset))
#str(base_data)
base_data
reporting_date
#Datum <- reporting_date
#Datum <- Sys.Date()
#valueBox(Datum,
#         color='blue')
#valueBox(base_data[base_date$DATUM==reporting_date,3],
#          color='blue')
#valueBox(reference_week[reference_week$weekday==weekdays(reporting_date),3],
#          color='blue')
```
